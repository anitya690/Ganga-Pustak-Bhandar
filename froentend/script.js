...
function renderBooks() {
  ...
  if (!filteredBooks.length) {
    bookGrid.innerHTML =
      '<div class="empty-state"><h3>No books found</h3><p>Search ko thoda change karke dobara try kariye.</p></div>';
    return;
  }
...
function openBookDetails(book) {
    ...
    <div class="checkout-panel">
      ...
      <div class="buy-box">
        <div>
          <span class="price">${formatMoney(book.amount)}</span>
          <span class="mrp">${formatMoney(book.mrp)}</span>
          <p class="dialog-note">Final availability shop confirmation ke baad lock hogi.</p>
        </div>
      ...
      <form class="order-form checkout-form" id="orderForm">
        ...
        <fieldset class="choice-grid">
          ...
          <label>
            ...
            <span>
              ...
              <small>Address par delivery confirm hogi</small>
            </span>
          </label>
        </fieldset>
        ...
        <div class="customer-grid">
          <label>
            Name
            <input name="customer_name" type="text" placeholder="Aapka naam" required>
          </label>
          <label>
            Phone
        ...
        <label>
          ...
          <textarea name="address" rows="3" placeholder="Delivery address ya pickup note" required>Pickup from store</textarea>
        </label>
  ...
  modalCartButton.addEventListener("click", () => {
    ...
    formMessage.textContent = "Cart me add ho gaya.";
  });
  ...
  orderForm.addEventListener("submit", async (event) => {
    ...
    formMessage.textContent = "Order place ho raha hai...";
    ...
    try {
      ...
      formMessage.textContent = `Order placed. Order ID: #${result.order_id}. Shop aapko confirmation call karegi.`;
      orderForm.reset();
      quantityInput.value = 1;
      updateSummary();
    } catch (error) {
      formMessage.textContent =
        "Order save nahi hua. Website ko python server.py se localhost par chalayen.";
    }
  });
